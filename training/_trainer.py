...
from . import prepare_splits as data_splits
...
def make_split_datasets(
    ...
    ``run_cross_validation.py`` supplies a fold; the ordinary training workflow supplies no fold and
    ...
    if config.fold is None:
        ...
        if not validation[0]:
            raise ValueError(
                "The manifest has no validation session. Add one with training/prepare_splits.py, "
...
def run_header(
    ...
    """Return the config plus the run facts checkpoint packaging needs.

    ``training/package_checkpoint.py`` reads these fields, so a packaged
