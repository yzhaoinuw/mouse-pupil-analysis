...
- `training/split_manager.py` provides a local drag-and-drop view of automatic session grouping,
  a live stacked pupil-size chart with median background brightness for the folds and a
  selectable session chart, plus validated assignment to folds or an optional validation holdout.
  Normal `run_train.py --split-manifest splits.json` runs validate against that holdout when it
  exists; when it is empty, they train all development sessions with a recorded fixed schedule
  instead of tuning against validation labels that do not exist.
