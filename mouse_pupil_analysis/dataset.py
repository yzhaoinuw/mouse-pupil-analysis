...
- :mod:`mouse_pupil_analysis.preprocessing` holds ``resize_with_pad`` and
  ``InferenceDataset``, the parts that run during inference.
- :mod:`mouse_pupil_analysis.augmentation` holds the training-time augmentations and
  ``SegmentationDataset``.
...
from mouse_pupil_analysis.augmentation import (
    RandomAffinePair,
    SegmentationDataset,
    random_pad_and_crop_pil,
    random_zoom_translate_pil,
)
from mouse_pupil_analysis.preprocessing import MODEL_IMAGE_SIZE, InferenceDataset, resize_with_pad
...
warnings.warn(
    "mouse_pupil_analysis.dataset is deprecated; import from mouse_pupil_analysis.preprocessing "
    "or mouse_pupil_analysis.augmentation instead.",
    DeprecationWarning,
    stacklevel=2,
)
