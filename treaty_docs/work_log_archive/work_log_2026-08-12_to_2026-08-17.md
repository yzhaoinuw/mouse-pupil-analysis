...
Sessions from 2026-08-12 through 2026-08-17, rotated from the live work log as a five-date group.
...
- Separated local media by purpose. `movies/` now contains only the three new source AVIs;
  ...
  the promoted README GIF; those extracted candidates and label picks now live in
  `frames_to_label/`.
...
- Added terminal argument parsing directly to `training/run_train.py`. Supplying arguments
  enables terminal use, while running with no arguments retains the editable direct-run
