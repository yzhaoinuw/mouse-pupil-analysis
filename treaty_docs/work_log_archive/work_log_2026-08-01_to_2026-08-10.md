...
Sessions from 2026-08-01 through 2026-08-10, rotated from the live work log as a five-date group.
...
- Added `TrackingAccumulator` in `tracking.py` and `MaskOverlayAccumulator` in `pupil_predictions.py`. The CLI now constructs those consumers only when their flags are enabled; `generate_pupil_predictions(...)` no longer accepts `calculate_velocity` or `acquisition_fps`. The tracking module now also has an editable direct-run block that leaves the detailed tracking DataFrame available for inspection.
- Reused the streamed binary mask during component measurement, preserving one UNet pass without repeating thresholding or accumulating all float probability maps. Kept the diameter-only convenience function, direct-run block, and original mask-prediction import path. Anchored both direct-run examples to the repository root instead of a machine-specific path or current working directory.
...
- Added a direct-run `if __name__ == "__main__":` block with editable local paths and inference settings. It runs `generate_pupil_predictions(...)` directly and writes example overlays without invoking the CLI parser.
